...
class WebSocketClient:
    ...
    def __init__(self, config: Dict[str, Any], security_manager: SecurityManager):
        ...
        # WebSocket连接信息
        self.ws_uri = config.get('proxy', {}).get('websocket_uri', 'ws://localhost:8765')
        self.api_key = config.get('proxy', {}).get('api_key')
        
        # 节点信息
        self.node_id = config.get('node', {}).get('id')
        self.node_name = config.get('node', {}).get('name', f'node-{self.node_id}')
        self.node_type = config.get('node', {}).get('type', 'cpu')
        self.max_workers = config.get('node', {}).get('max_workers', 4)
        ...
        # 心跳相关
        self.heartbeat_interval = config.get('proxy', {}).get('heartbeat_interval', 30)
        self.last_heartbeat = 0
        self.heartbeat_task = None
        
        # 重连相关
        self.reconnect_interval = config.get('proxy', {}).get('reconnect_interval', 5)
        self.max_reconnect_attempts = config.get('proxy', {}).get('max_reconnect_attempts', 10)
