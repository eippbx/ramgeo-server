...
class NodeServer:
    ...
    def __init__(self, config_path: str = None):
        ...
        # 代理服务器连接信息
        self.proxy_url = self.config.get('node_server.proxy.websocket_uri', 'ws://localhost:8080/ws')
        self.auth_token = self.config.get('node.auth_token')
        
        # 工作目录
        self.work_dir = Path(self.config.get('node.work_dir', './workspace'))
    ...
    async def start(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning("当前平台不支持信号处理，使用其他方式处理关闭")
            ...
            self.status = 'running'
            logger.info(f"节点服务启动成功，连接到: {self.ws_client.ws_uri}")
    ...
    async def _status_report_loop(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                # 收集系统指标
                system_metrics = await self.resource_monitor.get_resource_usage()
                ...
                # 发送状态报告
                await self.ws_client.send_node_status()
