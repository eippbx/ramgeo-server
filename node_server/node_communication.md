...
3. Node Server 发送 `register` 消息，包含 `node_id`
4. Proxy Server 验证 `node_id`的合法性
...
1. Proxy Server 每 30 秒（可配置 `websocket.heartbeat_interval`）向 Node Server 发送 `heartbeat` 消息
2. Node Server 收到心跳消息后，立即回复 `heartbeat_response` 消息
3. Proxy Server 如果在 60 秒（可配置 `websocket.heartbeat_timeout`）内未收到响应，标记节点为不健康
...
### 8.3 任务接收
- 接收 Proxy Server 发送的任务文件到该节点，在参数文件定义的runtime.work_dir目录下保存该文件

### 8.4 任务运行
- 节点在接收任务文件成功后，运行参数文件定义的runtime.run_bin指定的可执行文件，参数为任务文件的相对路径（相对于runtime.work_dir）
- 节点在运行此任务完成后，会在工作目录里自动生成两个文件：{任务文件名称}.grid、{任务文件名称}.line


### 8.5 任务完成
- 节点在任务执行完成后，应将生成的.grid、.line文件上传到代理服务器
- 上传文件时，文件路径为：{runtime.work_dir}/{任务文件名称}.grid、{runtime.work_dir}/{任务文件名称}.line
- 发送完文件后向代理服务器发送完成消息
```json
{
  "type": "task_complete",
