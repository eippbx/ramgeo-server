...
c
c     Added: Timing variables (不改变原有变量声明位置)
      real start_time, end_time, total_time
      integer range_steps, profile_updates, matrix_updates
      integer mdr_save, ndr_save, ndz_save, iz_save, nzplt_save
      integer lz_save, ib_save, ir_save
      real dir_save, dr_save, dz_save, pi_save, eta_save
      real eps_save, omega_save, rmax_save, c0_save, k0_save
      real r_save, rp_save, rs_save, freq_save, zs_save, zr_save
      real zmax_save
      complex ci_save
      integer nz_save, np_save, ns_save
c
      open(unit=1,status='old',file='ramgeo.in')
      open(unit=2,status='unknown',file='tl.line')
      open(unit=3,status='unknown',file='tl.grid')
c
c     Added: Start timing
      ...
      write(*,*) '=============================================='
      write(*,*) 'RAMGEO Version 1.5 - Starting Calculation'
      write(*,*) '=============================================='
c
     ...
     >   pd1,pd2,tlg)
c
c     Save parameters for final output
      mdr_save = mdr
      ndr_save = ndr
      ndz_save = ndz
      iz_save = iz
...
c     Read frequency, zs, zr from common block or save them
c     Note: These are read in setup subroutine
c     We'll need to save them separately
c
...
c     Added: Increment step counter
      range_steps = range_steps + 1
      
c     Added: Track updates
      if (r .ge. rp) then
          profile_updates = profile_updates + 1
      endif
      if (r .ge. rs) then
          matrix_updates = matrix_updates + 1
      endif
      
c     Added: Show progress every 100 steps
      if (mod(range_steps, 100) .eq. 0) then
          write(*,*) 'Progress: Range =', r, 'km, Steps =', range_steps
      endif
      
      call outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,tlg,k0)
...
c
      ...
      close(3)
c
c     Added: End timing and print statistics
      call cpu_time(end_time)
      total_time = end_time - start_time
      
      write(*,*) ' '
      write(*,*) '=============================================='
      write(*,*) 'RAMGEO Calculation Completed'
      write(*,*) '=============================================='
      write(*,*) 'RUNTIME STATISTICS:'
      write(*,*) '==================='
      write(*,*) 'Total CPU time (seconds): ', total_time
      write(*,*) 'Number of range steps:    ', range_steps
      write(*,*) 'Profile updates:          ', profile_updates
      write(*,*) 'Matrix updates:           ', matrix_updates
      
      if (range_steps .gt. 0) then
          write(*,*) 'Average time per step:    ', 
     >               total_time/real(range_steps), ' seconds'
      endif
      
      if (total_time .gt. 0.0) then
          write(*,*) 'Steps per second:        ', 
     >               real(range_steps)/total_time
          if (rmax_save .gt. 0.0) then
              write(*,*) 'Seconds per km:          ', 
     >                   total_time/(rmax_save/1000.0)
          endif
      endif
      ...
      write(*,*) '==================='
      write(*,*) 'Max Range (km):          ', rmax_save
      write(*,*) 'Grid Size (nz):          ', nz_save
      write(*,*) 'Pade terms (np):         ', np_save
      write(*,*) 'Depth step (dz):         ', dz_save
      write(*,*) 'Range step (dr):         ', dr_save
      write(*,*) 'Reference speed (c0):    ', c0_save
      write(*,*) 'Final range:             ', r_save
      write(*,*) '=============================================='
      
c     Write summary to log file
      open(unit=4, status='unknown', file='ramgeo_time.log')
      write(4,*) 'RAMGEO Runtime Summary'
      write(4,*) '====================='
      write(4,*) 'Total CPU time (s): ', total_time
      write(4,*) 'Wall clock time would require system_clock'
      write(4,*) 'Range steps:        ', range_steps
      write(4,*) 'Profile updates:    ', profile_updates
      write(4,*) 'Matrix updates:     ', matrix_updates
      write(4,*) 'Final range (km):   ', r_save
      write(4,*) 'Model parameters:'
      write(4,*) '  Max range (km):   ', rmax_save
      write(4,*) '  Grid size (nz):   ', nz_save
      write(4,*) '  Pade terms (np):  ', np_save
      write(4,*) '  Depth step (dz):  ', dz_save
      write(4,*) '  Range step (dr):  ', dr_save
      write(4,*) '  Ref speed (c0):   ', c0_save
      close(4)
      
      write(*,*) 'Summary written to ramgeo_time.log'
      write(*,*) '=============================================='
      write(*,*) ' '
c
      stop
...
c
	...
	real,allocatable::cwa(:)
	
c
      call zread(mz,nz,dz,cw)
	allocate(cwa(mz))

	call zread(mz,nz,dz,cwa)

c	write(*,*)(cwa(i),i=1,100)
c	pause
      call zread(mz,nz,dz,cb)
c	write(*,*)(cb(i),i=1,100)
c	pause
...
ccc 2016Äę08ÔÂ23ČŐ ĐŢ¸ÄÔöĽÓÁË ¸´ĘýĘäłö 
